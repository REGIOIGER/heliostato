...
#define M1CL  30 			//cm //Motor 1 Elevacion Cateto Largo
#define M1CC  24 			//cm //Motor 1 Elevacion Cateto Corto
#define M2CL  21.5   			//cm //Motor 2 Azimutal Cateto Largo
#define M2CC  17.5   			//cm //Motor 2 Azimutal Cateto Corto
#define D01   7   			//cm //longitud inicial de elevacion (d)
#define D02   11.5 			//cm //longitud inicial azimutal (d) 
...
unsigned char agEL,agAZ;		// posible uso para cambiar objetivo

unsigned char hora, minuto;		// Para modo demostracion

					// ANGULOS DEL OBJETIVO, GRADOS
int az = 200;
int el = 12;

float angEL;				// Angulo EL de prueba
float angAZ;				// Angulo AZ de prueba
...
//helios(bcdToDec(rxdata[5]), bcdToDec(rxdata[4]), hora, minuto, az, el);// modo demo 27 de agosto hora:minuto
helios(bcdToDec(rxdata[5]), bcdToDec(rxdata[4]), bcdToDec(rxdata[2]), bcdToDec(rxdata[1]), az, el);
...
int TZN;
	...
	//tAZ = ?; TODO: comentar bien que este es el angulo azimutal del objetivo
	tAZ = TAZ * PI/180;

	//tEL = ?; TODO: comentar bien que este es el angulo de elevacion del objetivo
	tEL = TEL * PI/180;
...
//angEL = deg2rad(i + 7.7); 0.134390 rad
//angAZ = deg2rad(i + 33);  0.567232 rad
	...
	} else
	{
		...
		//if(mEL > 0 && mEL < 1.047197)
		updateMotorEL(rad2len((mEL + 0.134390), M1CL, M1CC )); //revisa limites en radianes de el angulo de elevacion
		//if(mAZ > 2.356194 && mAZ < 3.926990)
		updateMotorAZ(rad2len((mAZ + 0.567232 - 2.356194), M2CL, M2CC )); //revisa limites en radianes de el angulo de azimutal
	}
...
{
	P1OUT &= ~(LED_0);
	n = (int)(d * 6.33 * 200);	//vueltas por unidad de longitud TODO relacion 6.33 rotaciones por cm, 200 pasos por rotacion
...
{
	P1OUT &= ~(LED_0);
	n = (int)(-d * 6.33 * 200);			    //vueltas por unidad de longitud
	UART_TX("\r\nNumero de pasos del motor de elevacion: "); 
	UART_TX("-");	
	UART_OutUDec(n);
	P1OUT &= ~(mElDir);		    //change direction of rotation
